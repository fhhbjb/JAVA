...
深拷贝意味着对一个对象进行**完全的复制**。它不仅复制了对象本身的字段值，而且如果对象内部包含对其他对象的引用（即嵌套对象），深拷贝会**递归地复制**这些被引用的嵌套对象，直到所有层级的对象都被独立复制。最终，原对象和新对象是**完全独立**的，修改其中一个不会影响另一个。


## 浅拷贝 (Shallow Copy) 与对象克隆 (Cloning)


    浅拷贝是一种“位对位”的复制。它会创建一个**新对象**，并将原对象的所有字段值复制到新对象中。
...
在 Java 中，数据类型主要分为两大类：**基本数据类型（Primitive Data Types）** 和 **引用数据类型（Reference Data Types）**。

**引用数据类型**是指那些不直接存储数据值本身，而是存储**对数据所在的内存地址的引用（即指针）**的数据类型。当你声明一个引用数据类型的变量时，你实际上是创建了一个指向某个对象的内存位置的“句柄”或“引用”。

Java中的引用数据类型主要包括以下几种：

1.  **类（Class）**：
    * 所有非基本数据类型的类都是引用数据类型。
    * 例如：`String`, `Scanner`, `ArrayList`, `Date`, `Thread` 以及你自己定义的任何类。
    * 当你创建一个类的对象时，实际上是在堆（heap）内存中分配了内存来存储对象的数据，而变量则存储了指向这个对象的引用。
    * 示例：`String s = "Hello";` (这里`s`是一个引用变量，指向存储“Hello”字符串的对象)
    * 示例：`MyClass obj = new MyClass();` (这里`obj`是一个引用变量，指向`MyClass`的一个实例)

2.  **接口（Interface）**：
    * 接口本身也是一种引用数据类型。
    * 你可以声明接口类型的变量，这些变量可以引用实现了该接口的类的对象。
    * 示例：`List<String> myList = new ArrayList<>();` (这里`myList`是`List`接口类型的引用，指向`ArrayList`对象)

3.  **数组（Array）**：
    * 在 Java 中，无论数组存储的是基本数据类型还是引用数据类型，数组本身都是一个对象，因此数组是引用数据类型。
    * 当你创建一个数组时，数组对象本身是在堆内存中分配的，而数组变量则存储了指向这个数组对象的引用。
    * 示例：`int[] numbers = new int[5];` (`numbers`是引用变量)
    * 示例：`String[] names = new String[3];` (`names`是引用变量)

**引用数据类型的特点：**

* **存储的是引用：** 变量本身存储的不是实际的数据值，而是数据值在内存中的地址。
* **内存分配在堆（Heap）：** 引用数据类型所引用的对象通常存储在堆内存中。
* **默认值为 `null`：** 如果一个引用数据类型的变量没有被初始化（即没有引用任何对象），它的默认值是 `null`。`null`表示该引用不指向任何对象。
* **通过引用操作对象：** 你需要通过引用变量来访问或操作它所指向的对象的数据和方法。
* **垃圾回收（Garbage Collection）：** 当一个对象不再有任何引用指向它时，Java的垃圾回收器会自动回收其占用的内存。

**与基本数据类型的对比：**

| 特点           | 基本数据类型                               | 引用数据类型                                   |
| :------------- | :----------------------------------------- | :--------------------------------------------- |
| 存储方式       | 直接存储数据值                             | 存储数据的内存地址（引用）                     |
| 内存分配位置   | 通常在栈（Stack）上（局部变量）             | 通常在堆（Heap）上                             |
| 默认值         | 有默认值（如 `int` 为 0, `boolean` 为 `false`） | `null`                                        |
| 内存大小       | 固定大小（如 `int` 4字节）                  | 大小通常固定，因为它存储的是地址（取决于JVM），但其引用的对象大小不固定 |
| 传递方式（方法参数） | 值传递（复制一份值）                       | 值传递（复制一份引用，但指向的是同一个对象）   |
