...
### 理解 Java 中的 `Object` 类与 `toString()` 方法
 ...
 Java 中的一个基本概念：所有类都直接或间接地继承自 `Object` 类，因此也继承了 `Object` 类的方法，包括 `toString()` 方法。
...
Java
     变量：都是静态绑定的。
     方法 ：只有==static==和==final==(所有private默认是final的) 是静态绑定的，其余方法是动态绑定的。
...
**定义：**
向上转型是指将一个子类对象引用赋值给一个父类类型的引用变量。father=son
...
* **运行时行为：** 即使通过父类引用调用方法，如果该方法被子类重写（Override）了，运行时仍然会执行子类中重写后的方法（这是==动态绑定==的体现）。
...
**定义：**
向下转型是指将一个父类类型的引用变量强制转换为一个子类类型的引用变量。 son=father

**特点：**
* **显式（强制）转型：** 向下转型==必须使用强制类型转换符 `()`==，因为 Java 编译器无法保证转换的安全性。
...
#### 运行时多态的详细解释：![alt text](image-12.png)
...
软件建模的时候，如果无法确定类的某些方法的具体实现，那么该方法需定义为抽象方法，该类需定义为抽象类。然后在子类中实现这些抽象方法。
![alt text](image-13.png)
#### 1. 抽象类的定义

要定义一个抽象类，需要使用 `abstract` 关键字修饰类。
`static` 、`private` 和`final`修饰词不能修饰抽象方法和抽象类 。因为这些修饰词都是为了不能修改。抽象类存在的目的主要是为了被其他类继承，作为其子类的通用模板或基础骨架。
...
要定义一个接口，需要使用 `interface` 关键字。![alt text](image-14.png)
...
```
**notice:**
1. 常量的修饰符必须为public static final，默认。
2. 成员方法必须是抽象方法，默认。
...
```
**Notice:**
1类在实现抽象方法时，必须用public修饰符。　　
2如果类没有实现所有接口的方法，则该类就必须被声明成抽象类。
```java
...
#### 接口的继承
接口的继承：子接口将继承父接口的所有属性和方法。
     **notice**:一个接口可以继承多个接口,实现java语言的多继承
格式：`public interface 子接口 extends 父接口1,
                       父接口2, 父接口3{
                  //接口成员常量和抽象方法声明
　　　}`
```java
...
### 编译与运行某个包中的类
        ==编译==：     javac     类
         ==运行==:      java    包.类
### JVM寻找类的过程
`D:save>Java     c.TestPackage`
到classpath中设置的每个目录中去寻找该类。
          当然如果该类在某个包中，则该包也需要对应classpath中的某个目录的同名子目录中。

<p>1: 在c:\save目录下创建三个子目录: a, b, c</p>
<p>2: 把编辑好的源文件T1.java、T2.java和T1.java(与前一个T1.java不同)，以及TestPackage.java放入所对应的子目录(包)下。</p>
<p>3: 分别编译各子目录下的源文件。</p>
<pre>
c:\save\a>javac T1.java
c:\save\a>javac T2.java
c:\save\b>javac T1.java
c:\save\c>javac TestPackage.java
</pre>
<p>4: 运行测试文件</p>
<pre>
c:\save\java c.TestPackage
c:\save\java TestPackage
c:\save\c>java c.TestPackage
c:\save\c>java TestPackage
d:\> java c.TestPackage
d:\> java TestPackage
</pre>

4:　运行测试文件
　　d:\save\>java   c.TestPackage
       d:\save\>java  TestPackage   ==错==
       d:\save\c>java  c.TestPackage 对
       d:\save\c>java  TestPackage  ==错==
       c:\> java  c.TestPackage 对
       c:\> java   TestPackage  ==错==

如果classpath中==不包含d:\save ==
c:\save\>java   c.TestPackage  
c:\save\>java  TestPackage    
c:\save\c>java  c.TestPackage  
c:\save\c>java  TestPackage   
...
如果classpath中不包含d:\save 
d:\save\>java   c.TestPackage 对
d:\save\>java  TestPackage   ==错==
d:\save\c>java  c.TestPackage ==错==
d:\save\c>java  TestPackage  ==错==
c:\> java  c.TestPackage==错==
c:\> java   TestPackage  ==错==

<p>步骤：</p>
<p>1: 把编辑好的源文件T1.java、T2.java和T1.java(与前一个T1.java不同)，以及测试文件放入d:\save下。</p>
<p>2: 分别编译各源文件。</p>
<pre>
d:\save>javac -d . T1.java
d:\save>javac -d . T2.java
d:\save>javac -d . T1.java
d:\save>javac -d . TestPackage.java
</pre>
<div style="border: 2px solid red; padding: 10px; margin-top: 10px; margin-bottom: 10px;">
  <p>自动创建三个包（子目录）：</p>
  <p>a、b、c，并且把各个字节码文件放入相应的包下</p>
</div>
<p>或：</p>
<div style="border: 2px solid blue; padding: 10px; margin-top: 10px; margin-bottom: 10px;">
  <pre>d:\save>javac -d . *.java :联编</pre>
</div>
<p>3:运行测试文件 d:\save\java c.TestPackage</p>
